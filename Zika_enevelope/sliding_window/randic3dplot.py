...
def sliding_window(seqstart, seqlen, seqarr, window_list):
	...
	for i in range(seqstart, seqstart+seqlen):
		...
		for j in range(i, seqstart+seqlen):
			...
			edmat[i - seqstart][j - seqstart] = sqrt(pow((seqarr[i].x - seqarr[j].x), 2) + pow((seqarr[i].y - seqarr[j].y),2) + pow((seqarr[i].z - seqarr[j].z),2)) / pow(3, 0.5)
	...
	window_list.append(round(np.linalg.eigvals(mbym_mat).max(), 4))
