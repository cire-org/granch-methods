...
from nandyplot_err import nandyplot_err
...
import os
path = '/home/dwaipayan/Dropbox/GRANCH Review/Progs_and_Results/data/virus_envelopes'
files = []
for i in os.listdir(path):
    if i.endswith('.txt'):
    	#files.append(open(i))
		files.append(i)
# for name in files:
# 	nandyplot_err(path + "/" + name)
# 	print("nandy")
# 	nandyplot(path + "/" + name)
# 	#jiliplot(path + "/" + name)
# 	# nandyplot(path + "/" + name) 

nandyplot_err("test.txt")
# print("nandy")
# nandyplot("test.txt")